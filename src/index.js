...
let modifedDocx = path.join(__dirname, `../modified_document.docx`);
const audienceAttr = require("./utils/audienceAttr.js")
...
async function convertDocxToDita(filePath) {
  ...
  try {
    ...
    const parsedXml = await xml2js.parseStringPromise(docXml);

    const colorAudienceMapping = {
      '999999': 'Audience1',
      '0033FF': 'Audience2'
    };
    ...
    function processParagraphs(paragraphs) {
      paragraphs.forEach(paragraph => {
        ...
        if (paragraph['w:hyperlink']) {
          paragraph['w:hyperlink'].forEach(hyperlink => {
            if (hyperlink['w:r']) {
              hyperlink['w:r'].forEach(run => {
                if (run['w:rPr'] && run['w:rPr'][0]['w:color']) {

                  const color = run['w:rPr'][0]['w:color'][0]['$']['w:val'];
              
                  if (colorAudienceMapping[color]) {
                    audience = colorAudienceMapping[color];
                  }
                }
              });
            }
        ...
        if (audience) {
          ...
          paragraph['w:pPr'][0]['w:pStyle'] = [{ '$': { 'w:val': audience } }];
        }
      });
    }

    if (parsedXml['w:document'] && parsedXml['w:document']['w:body']) {
      processParagraphs(parsedXml['w:document']['w:body'][0]['w:p']);
    }
    ...
    fs.writeFileSync('modified_document.docx', newDocxData);
...
// const runs = docXml.getElementsByTagName('w:r');

// // Process each run
// Array.from(runs).forEach(run => {
//   const texts = run.getElementsByTagName('w:t');

//   const colorElements = run.getElementsByTagName('w:color');
...
//     audience = getAudience(color);
//   }

//   Array.from(texts).forEach(text => {
//     if (audience) {
//       const audienceAttr = docXml.createAttribute('audience');
//       audienceAttr.value = audience;
//       text.setAttributeNode(audienceAttr);
//     }
//   });
// });
...
// const modifiedXmlContent = serializer.serializeToString(docXml);
...
// fs.writeFileSync("modified_document.docx", newZipContent);
    ...
    const mammothOptions = {
      styleMap: [
          ...
          "p.Audience1 => a > audience1:fresh",
          "p.Audience2 => a > audience2:fresh"
